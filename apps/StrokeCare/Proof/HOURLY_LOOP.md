...
## 2026-08-28 18:44 SGT — one consolidated family entry cue

- Target: remove the remaining duplicate family onboarding object while keeping the generic brain, lesson points and three-act Pressure story immediately understandable. Budget: one visual-hierarchy correction on `latest-arnav-build`; no `main` merge, paid provider use, new clinical content or release claim.
- Action: merged the ceiling-height `OUTSIDE THE BRAIN` card into the existing `BEGIN HERE` cue beside the brain. The single compact surface now identifies the generic whole-brain teaching model and points to one glowing lesson point. It disappears when a point or Atlas lesson takes over. The family surface uses a restrained dark backing for contrast in bright rooms; the clinician explanation rail retains its thin-material treatment. Existing point input targets, collision fields, hover effects and accessibility labels were preserved rather than duplicated.
- XCAT gate: `Scripts/deploy_xcat.zsh` stopped safely because `XCAT.coredevice.local` / device `613CC48C-A6AD-5170-A238-D518B6012491` was unavailable. Receipt: `Proof/xcat/20260828-183427/BLOCKED.md`. No device build, install, launch, wearer-visibility or gesture claim is made.
- Verification: `python3 Tests/verify_contract.py` and `git diff --check` pass. The narrow visionOS Simulator build succeeds against `F8B7E8FD-DBF2-4270-A6FD-2BA02CD6F777`; ad-hoc signing and `codesign --verify --deep --strict` pass. Version remains `0.6 (29)`. Rendered app SHA-256: `9e6966f02982a5dcce823a09268ca5f137936de43913f2d935b565200a6d9556`.
- Visual evidence: `Proof/112-single-family-entry-cue-simulator.png` passes route OCR 2/2 for `outside the brain` and `begin here`; SHA-256 `19116e6430d5dc8b07a344f1d2c455c2ca55b560fb7293cbc62d54ba94e2c527`. Inspection shows one compact cue beside the dominant brain, no ceiling card and the timeline and family rail unobstructed. A fresh clinician Pressure regression also passes route OCR 2/2 and preserves the established left explanation, right reference rail and single timeline; its temporary image SHA-256 is `eeb7d6abfd52cc750cff8f88e9aec710da1f5a9019f0e790d0a830303b007585`.
- Error and correction: the first family capture rendered the correct single-card structure, but OCR missed `BEGIN HERE` because the thin material was too faint against the bright room. That receipt was not accepted. The family card received a darker backing and restrained accent stroke; the rebuilt, recaptured route then passed without weakening the image or route thresholds.
- Accuracy boundary: this is generic teaching anatomy and an interface hierarchy change. It does not establish patient-specific anatomy, diagnostic meaning, headset reach, physical pinch quality, wearer comprehension, stereo comfort, clinical acceptance or App Store readiness.
- Verdict: `IMPROVED :)` — the family entry now has one spatial starting place instead of two detached instructions, with clearer contrast and no added control clutter. The broader product goal remains active.
- Blocker: XCAT is unavailable, and Simulator receipts cannot establish wearer or specialist acceptance.
- Next safe action: on Apple Vision Pro, have one family learner locate and pinch the first glowing point from this consolidated cue, then record the first ambiguity in wording, placement or acquisition.
- Layman equivalent: families now see one small dark hint beside the brain telling them where they are and what to touch first. The Mac test can see and read it reliably, but a person still needs to try that first pinch in the headset.

## 2026-08-28 19:07 SGT — each teaching act owns its relevant spatial points

- Target: stop the clinician timeline from reusing unrelated points and detached skull cues across acts. Budget: one timeline-to-anatomy interaction correction on `latest-arnav-build`; no `main` merge, paid provider use, operative simulation or new clinical claim.
- Action: renamed the clinician's second checkpoint to the visible **Pressure** story and bound it to the five existing vessel-story anchors. The first **Make Space** checkpoint now owns the single craniotomy invitation. Later checkpoints clear the point field instead of leaving stale dots active. A pre-Permission access pinch advances to the permission-controlled protective-covering checkpoint. The skull context moved with that checkpoint and no longer appears during Pressure.
- Visual correction: the first Make Space capture exposed the old twelve-dot pressure contour beside the one orange access cue. The render was not accepted. The clinician pressure story is now scoped to the Pressure checkpoint, so the rebuilt Make Space frame contains one access invitation without the inherited mint cluster. Family mode retains its separate, calm region-point opening.
- Errors and correction: the first contract update still expected the former `discussAccess` skull ownership and failed. The source and contract were aligned to `protectiveCovering`; a later contract edit initially referenced two unread proof-harness variables, so the proof script and verifier were added explicitly to the contract input set. The corrected contract passes.
- XCAT gate: `Scripts/deploy_xcat.zsh` stopped before build, install or launch because paired device `613CC48C-A6AD-5170-A238-D518B6012491` was unavailable with no tunnel or DDI services. Receipt: `Proof/xcat/20260828-185444/BLOCKED.md`. No physical-device result is claimed.
- Verification: `python3 Tests/verify_contract.py` and `git diff --check` pass. The narrow visionOS Simulator build succeeds against `F8B7E8FD-DBF2-4270-A6FD-2BA02CD6F777`; ad-hoc signing and `codesign --verify --deep --strict` pass. Version remains `0.6 (29)`. Rendered app/debug-library SHA-256: `17aa5f6a928d3482ea83cfe14c06ce74b26478c8a2fbcc94bc581437bccaf703`.
- Visual evidence: `Proof/113-pressure-owned-flow-points-simulator.png` passes route OCR 2/2 and was inspected with five flow anchors, the Flow explanation selected, one continuous timeline and no access skull; SHA-256 `0c223e9c4543dd4a03b40aedeaa736547c5c76887b3ea6ddc491df7c9c474bdf`. `Proof/114-make-space-owned-access-point-simulator.png` passes route OCR 2/2 and was inspected with one orange access invitation and no inherited pressure contour; SHA-256 `800b987d09a6e2a4659a8444861202ece12a26fa54be0c25efb5e6cc9f4a3b23`. A fresh family regression passes route OCR 2/2 on the same app and preserves the broad-region `BEGIN HERE` composition; temporary image SHA-256 `ba1505ab7432a54c8fb27e20ff7250a8041714405cb2a249520effaadfc7f5c6`.
- Accuracy boundary: vessel anchors, affected-tissue cue and pressure contour are generic qualitative teaching relationships. They do not encode CFD, pressure, edema volume, a measured lesion, diagnosis, prognosis, patient anatomy or an operative access plan. The access study remains non-graphic, reversible and specialist-review pending.
- Verdict: `IMPROVED :)` — timeline position now changes both wording and the relevant spatial interaction family, while the two most easily confused clinician acts no longer display each other's cues.
- Blocker: XCAT is unreachable, and Simulator receipts cannot establish physical gaze acquisition, pinch reliability, stereo placement, wearer comprehension, anatomical registration or clinical validity.
- Next safe action: on Apple Vision Pro, pinch all five Pressure points and then the single Make Space access point, recording the first target whose position or meaning is unclear.
- Layman equivalent: the pressure lesson now shows only the five dots that explain blood supply and blockage. When the presenter moves to making space, those dots disappear and one orange dot opens the layer lesson. The Mac proves the two screens are different and readable; a person still needs to try them in the headset.

## 2026-08-28 19:19 SGT — loaded scans own the imaging gallery

- Target: make the default Imaging Gallery image-first instead of wasting half of its focused workspace on empty comparison slots and disabled single-page controls. Budget: one spatial hierarchy correction on `latest-arnav-build`; no `main` merge, patient upload, paid provider use or new clinical content.
- Action: 2 × 2 now behaves as a maximum comparison density. With the two bundled research-atlas scans, CT and MRI become two full-height side-by-side plates. Real images are rendered directly rather than padded to grid capacity, and Previous, page count and Next appear only when another page exists. The 2 × 2, 3 × 3 and 4 × 4 density choices, modality filter, Add scans flow, per-image detail, annotation and placement route remain intact. A zero-result filter retains one calm, full-surface Add images state.
- Error and correction: the first build process ended during asset compilation without producing an app bundle, so it was not accepted. Re-running the identical incremental build completed with `BUILD SUCCEEDED`; no source workaround or deployment-target change was needed.
- XCAT gate: `Scripts/deploy_xcat.zsh` stopped before build, install or launch because paired device `613CC48C-A6AD-5170-A238-D518B6012491` was unavailable, with no tunnel or DDI services. Receipt: `Proof/xcat/20260828-191138/BLOCKED.md`. No physical-device result is claimed.
- Verification: `python3 Tests/verify_contract.py` passes, including a guard against capacity-filling placeholders and one-page navigation. The standalone gallery verifier passes 30 checks, and `git diff --check` passes. The narrow visionOS Simulator build succeeds against `F8B7E8FD-DBF2-4270-A6FD-2BA02CD6F777`; ad-hoc signing and `codesign --verify --deep --strict` pass. Version remains `0.6 (29)`. Built executable SHA-256: `80d2c01bb016e75c29efa3199f75c63de5dbbed2c7f73506e2d6517f762f58e4`.
- Visual evidence: `Proof/115-image-first-imaging-gallery-simulator.png` passes the unchanged image-quality gate and route OCR 4/4; SHA-256 `3d1a388953a156d61d75f51e46ab5e64fa8e0547cbca81b84df7524f830009e2`. Inspection shows two substantially larger scan plates, one compact control row, no empty lower tiles and no disabled pagination strip.
- Accuracy boundary: both bundled images remain de-identified research-atlas teaching references. They are independent, not registered to the 3D brain, not patient scans and not interpreted findings. This change does not add diagnosis, segmentation, registration, prognosis or image-analysis claims.
- Verdict: `IMPROVED :)` — the primary scan content now dominates its focused workspace while comparison controls remain ready for larger collections. The broader product goal remains active.
- Blocker: XCAT is unreachable, and Simulator receipts cannot establish physical gaze-and-pinch acquisition, headset readability, wearer comfort, radiology workflow fit or clinical validity.
- Next safe action: on Apple Vision Pro, open both atlas scans and switch once through 2 × 2, 3 × 3 and 4 × 4, recording the first plate or density target that is hard to acquire or read.
- Layman equivalent: the scan room no longer shows two useful pictures plus two giant empty boxes. The two pictures now fill the space, and page buttons only appear when there is another page to visit. The Mac proves the layout renders; someone still needs to try it in the headset.

## 2026-08-28 19:44 SGT — the presenter speaking aid follows all six checkpoints

- Target: make the left doctor-presenter guidance change with the six-step timeline instead of reusing one generic Access vocabulary. Budget: one focused presenter interaction pass on `latest-arnav-build`; no `main` merge, paid provider use, generated clinical wording or new patient claim.
- Action: authored a distinct technical-to-plain vocabulary for Confirm context, Pressure, Protective covering, Make room, Checks and Closure. The first phrase opens automatically at each checkpoint; the remaining phrases reveal one short explanation in place. The left surface is 460 points wide and slightly farther from the brain, while the shared anatomy remains central. Presenter status now says `CHECKPOINT n OF 6` in both the left speaking aid and lower control rail; family status remains `ACT n OF 3`.
- Accuracy boundary: the copy distinguishes generic teaching anatomy from patient imaging and explicitly avoids measurements, diagnosis, prognosis, treatment ranking, eligibility, operative site planning and outcome promises. It is authored and deterministic; it is not generated or adapted from a face, voice, scan or inferred emotion. Clinical review remains pending.
- Error and correction: the first contract run failed because one old footprint check still required the former 410-point panel. The check was updated to the intentional 460-point size. The first Covering screenshot then exposed mixed numbering, with `CHECKPOINT 3 / 6` on the left and `ACT 3 OF 3` on the presenter rail. That receipt was replaced after making the rail role-specific and rebuilding. A final standalone image-check command initially passed a leading-dash route as a separate argument; rerunning with `--route=...` passed both unchanged images.
- XCAT gate: `Scripts/deploy_xcat.zsh` stopped before build, install or launch because paired device `613CC48C-A6AD-5170-A238-D518B6012491` was unavailable. Receipt: `Proof/xcat/20260828-192259/BLOCKED.md`. No physical-device result is claimed.
- Verification: `python3 Tests/verify_contract.py` and `git diff --check` pass. The narrow visionOS Simulator build succeeds against `F8B7E8FD-DBF2-4270-A6FD-2BA02CD6F777`; ad-hoc signing and `codesign --verify --deep --strict` pass. Version remains `0.6 (29)`. Rendered app/debug-library SHA-256: `5800ee811f6bf47e5d3c729e5d0251c1c547f29ec0474aa3cef169e64d81347c`.
- Visual evidence: `Proof/116-six-checkpoint-pressure-language-simulator.png` passes route OCR 4/4 for `explain this`, `pressure`, `occlusion` and `swelling`; SHA-256 `01be2e4945ced5898e53a455e4816fe165892a5c4e70d1826112ae2e723e7152`. `Proof/117-six-checkpoint-covering-language-simulator.png` passes route OCR 2/2 for `explain this` and `covering`; SHA-256 `faff1561afbbdac0f9a31fde03f0b36f76314f02975738b6acf7d39b57ad1857`. Inspection confirms the first frame shows Blood supply, Occlusion, Affected tissue and Swelling, while the second changes to Skull, Bone flap, Dura and Teaching boundary without adding another panel.
- Verdict: `IMPROVED :)` — the timeline now changes the presenter's actual speaking aid, not only the anatomy state, and the two presenter surfaces agree on one six-checkpoint structure.
- Blocker: Simulator receipts cannot establish physical readability, gaze-and-pinch acquisition, stereo placement, family comprehension, specialist acceptance or clinical validity.
- Next safe action: on Apple Vision Pro, have one clinician visit all six checkpoints and select one term at each, recording the first phrase or target that is unclear.
- Layman equivalent: the doctor's note card now follows the lesson. The pressure step talks about blood supply and blockage; the next step talks about the skull and protective covering. Only one explanation opens at a time, so the screen stays calm.

## 2026-08-28 20:13 SGT — one family point owns its follow-up conversation

- Target: let a family learner ask the obvious question and say whether the answer helped without leaving the selected anatomy point or opening another floating surface. Budget: one focused family interaction pass on `latest-arnav-build`; no `main` merge, paid provider use, inferred anxiety, patient data or generated clinical wording.
- Action: after the learner opens **Plain words**, the repeated technical summary now steps aside. The same point-owned card reveals two authored neuron questions, one in-place authored answer, and three stable 44-point clarity choices: **Again**, **Unsure**, and **Clear**. The 3D neuron remains in the opposite secondary field and the central registered brain remains dominant. Apple HIG progressive-disclosure and stable-target guidance informed the single-card structure and gaze-sized controls.
- Verification: `python3 Tests/verify_contract.py` and `git diff --check` pass. Both the diagnostic no-sign build and the normal local-signing visionOS Simulator build succeed against `F8B7E8FD-DBF2-4270-A6FD-2BA02CD6F777`. The signed bundle passes `codesign --verify --deep --strict`; debug-library SHA-256 `1e559d48f9f056d70a37c777fd7778f59a40e2d4fd44450ed244185697fc095d`. Version remains `0.6 (29)`.
- Visual evidence: `Proof/118-selected-point-family-follow-up-simulator.png` passes the unchanged image-quality gate and route OCR 4/4 for `plain words`, `one neuron`, `ask next`, and `did this make sense`; SHA-256 `516264fcac534a949b4f8a22df9a865da5a6be75956c8fbc21d8d306e6517ba1`. Inspection shows one dominant brain, one point-local explanation card, one separate neuron model, the family timeline, and no duplicate question window.
- Error and correction: strict verification of the intentionally no-sign diagnostic bundle failed because its resources were not covered by a valid final signature. That bundle was not accepted as signing proof. A normal Simulator build produced a local ad-hoc signature and passed strict verification. The first clarity layout hyphenated **Unsure**; its label was constrained to one line, rebuilt, and recaptured before acceptance.
- XCAT gate: `Scripts/deploy_xcat.zsh` stopped before build, install or launch because `XCAT.coredevice.local` / paired device `613CC48C-A6AD-5170-A238-D518B6012491` was unavailable. Receipt: `Proof/xcat/20260828-195020/BLOCKED.md`. No headset result is claimed.
- Accuracy boundary: the neuron and color explanations are generic authored teaching text. They do not represent recorded electrical activity, a patient measurement, diagnosis, prognosis or treatment advice. Clarity is explicitly selected by the learner and is never inferred from face, gaze, voice or physiology.
- Verdict: `IMPROVED :)` — the selected lesson now supports one coherent question-and-response loop without adding another panel or weakening the central spatial anatomy.
- Blocker: Simulator evidence cannot establish physical gaze-and-pinch acquisition, stereo depth, wearer comfort, family comprehension, device audio or specialist acceptance; XCAT is unavailable.
- Next safe action: on Apple Vision Pro, have one family learner open **Plain words**, select both follow-ups, and choose each clarity response once, recording the first target or phrase that is hard to acquire or understand.
- Layman equivalent: after touching the neuron, the learner can now ask two natural questions and tell the presenter whether the explanation was clear, all in the same small card beside the brain. The Mac proves the layout renders and the buttons are wired; a person still needs to try it in the headset.

## 2026-08-28 20:29 SGT — the selected point responds to explicit clarity

- Target: make **Again**, **Unsure**, and **Clear** visibly useful after a family learner selects one, without adding another panel or inferring emotion. Budget: one point-owned response improvement on `latest-arnav-build`; no `main` merge, paid provider use, patient data, generated medical wording or release claim.
- Action: each explicit clarity choice now replaces the previous response with one short authored cue in the same selected-point card. The deterministic neuron route selects **Unsure** and points the learner back to the teal branches and violet fiber. Apple HIG progressive-disclosure and stable-target guidance informed the in-place response and retained 44-point choices. The response is session-local self-reporting, never face, gaze, voice or physiology inference.
- XCAT gate: `Scripts/deploy_xcat.zsh` stopped before build, install or launch because paired device `613CC48C-A6AD-5170-A238-D518B6012491` was unavailable. Receipt: `Proof/xcat/20260828-201854/BLOCKED.md`. No physical-device result is claimed.
- Verification: `python3 Tests/verify_contract.py`, `git diff --check`, `zsh -n Scripts/capture_simulator_route_proof.zsh`, and `python3 -m py_compile Tests/verify_proof_image.py` pass. The narrow visionOS Simulator build succeeds against `F8B7E8FD-DBF2-4270-A6FD-2BA02CD6F777`; the locally signed bundle passes `codesign --verify --deep --strict`. Version remains `0.6 (29)`. Debug-library SHA-256: `8beb1c17c0d261caea9bd3ca3050c3dd0276eee6af55c8b323c0efd13d2f13ca`.
- Visual evidence: `Proof/119-selected-point-unsure-response-simulator.png` passes the unchanged image-quality gate and route OCR 4/4 for `one neuron`, `ask next`, `still unsure`, and `teal branches`; SHA-256 `4e92bf21f18251e4408b53a738b0f924f651d2e465fada596279de98b4371ecf`. Inspection shows one dominant brain, one point-owned explanation card, one separate neuron, the family rail and one three-act timeline, with no new floating surface or clipping.
- Error and correction: the first implementation passed the route and image gates, but visual inspection found its clarity response too small and wordy. It was not accepted. The copy was shortened, the type increased from caption2 to caption, and the app was rebuilt and recaptured before acceptance.
- Accuracy boundary: the neuron and response are generic authored teaching material. They do not represent exact neurophysiology, recorded activity, a patient scan, diagnosis, treatment, measured comprehension or clinical advice.
- Verdict: `IMPROVED :)` — an explicit family response now produces immediate, nearby guidance while preserving the calm central anatomy composition.
- Blocker: XCAT is unavailable, and Simulator receipts cannot establish physical gaze-and-pinch acquisition, stereo depth, wearer comfort, comprehension, device audio or specialist acceptance.
- Next safe action: on Apple Vision Pro, have one family learner select **Again**, **Unsure**, and **Clear** at the neuron point and record the first response or target that is hard to acquire or understand.
- Layman equivalent: if someone says they are still unsure, the same card now gives one short direction about what to look at next. It does not guess how they feel, and it does not cover the room with another window.

## 2026-08-28 20:51 SGT — clarity no longer follows the learner to another point

- Target: stop an explicit family clarity response and its temporary pause from leaking from one anatomy point into the next. Budget: one point-state correction on `latest-arnav-build`; no `main` merge, paid provider use, patient data, emotion inference, new clinical wording or unreviewed asset intake.
- Action: made **Again**, **Unsure**, and **Clear** point-owned. Moving to a different point clears the previous selection and response. **Again** or **Unsure** may hold authored motion, while **Clear** releases only a hold created by that clarity response. A deliberate Pause action and a separately placed question keep independent ownership. Added a deterministic neuron-Unsure to affected-area regression route and stale-content rejection.
- XCAT gate: `Scripts/deploy_xcat.zsh` stopped before build, install or launch because `XCAT.coredevice.local` / paired device `613CC48C-A6AD-5170-A238-D518B6012491` was unavailable. Receipt: `Proof/xcat/20260828-203246/BLOCKED.md`. No physical-device result is claimed.
- Verification: `python3 Tests/verify_contract.py`, `git diff --check`, `zsh -n Scripts/capture_simulator_route_proof.zsh`, and Python compilation pass. The visionOS Simulator build exits 0 for `F8B7E8FD-DBF2-4270-A6FD-2BA02CD6F777`; `codesign --verify --deep --strict` passes. Version remains `0.6 (29)`. Executable SHA-256: `e9d9267401747ddf74d2accd1013ea3d8103c33e435877f5019c551fd72662e1`.
- Visual evidence: `Proof/120-family-point-conversation-reset-simulator.png` passes the unchanged image-quality gate and route OCR 4/4; SHA-256 `10f3be3b56c657534864aa05f9d0ebcbcde45e21e7e55b264ee1d00615851e8e`. Inspection confirms one dominant brain, one affected-area card, one arterial reference, an unanswered clarity row, a visible Pause control, and no inherited `Still unsure` response.
- Error and correction: the first image gate expected the retired phrase `What this opens`, while the current reviewed card visibly used `Example affected area`. The screenshot was inspected rather than accepted blindly, the proof contract was corrected to the current authored label, and the same fresh frame passed. A manual verifier invocation also initially passed a leading-dash route as a separate argument; rerunning it as `--route=...` passed.
- Accuracy boundary: this is generic authored teaching anatomy and explicit session-local feedback. It does not infer anxiety or comprehension, interpret a patient scan, establish a diagnosis, model treatment, or validate neuroanatomical registration.
- Verdict: `IMPROVED :)` — each point now begins its own calm conversation instead of inheriting another point's answer or pause.
- Blocker: XCAT is unavailable, and Simulator receipts cannot establish physical gaze-and-pinch acquisition, stereo depth, wearer comfort, comprehension, specialist acceptance or clinical validity.
- Next safe action: review the license, provenance, polygon budget, scale and RealityKit compatibility of one high-detail neuron USDZ, then replace the current schematic teaching reference only if it passes those gates.
- Layman equivalent: saying “I’m unsure” at the neuron no longer makes the next brain lesson look unsure or paused. Each dot starts fresh, while a pause the person chose themselves stays respected. The Mac proves the state and layout; a headset wearer still needs to try it.

## 2026-08-28 21:31 SGT — the neuron reference is now a bundled biological USDZ

- Target: replace the symbolic procedural neuron with one detailed, named, RealityKit-compatible nerve-cell teaching asset without bulk-loading unreviewed geometry. Budget: one cellular-reference upgrade on `latest-arnav-build`; no `main` merge, paid asset, patient data, operative simulation, device claim or clinical claim.
- Action: authored an original deterministic Blender neuron, consolidated it to nine semantic meshes, packaged it as a 1,022,506-byte USDZ, and made that bundle resource the normal point-owned reference. It contains a soma, nucleus, branching dendrites, a spine field, axon hillock and core, segmented myelin, nodes of Ranvier, and a terminal arbor. The old procedural construction is retained only as a visible load-failure fallback. Runtime amber motion remains an explicitly qualitative teaching overlay rather than baked or measured neural activity.
- Verification: `usdchecker --arkit` passes for the source USDC and USDZ. `python3 Tests/verify_contract.py` returns `STROKE_CARE_CONTRACT=PASS`; `git diff --check` passes; the narrow visionOS Simulator build succeeds. A clean signed build with Derived Data under `/tmp` also succeeds and passes `codesign --verify --deep --strict`. USDZ SHA-256: `0d4a34b45f3cc94c5c04bb344c5fdd4c050dd9010d6822f3941ebc3f855f8459`; final signed-build executable SHA-256: `3b33d7f20e4f66d321331a918372ec7ef77f67deee150a21774b281902b047f5`; captured-app executable SHA-256: `d37808c5713f271e085b70540a0278d221aaf4e662aa62ec40865c76c907ba0c`.
- Visual evidence: `Proof/121-family-detailed-neuron-usdz-simulator.png` passes the unchanged image-quality gate and route OCR 2/2 at 3840×2160; SHA-256 `b99557db6370008caaf8a171e8e93d1ef6de39ea67e80d69045887ef719a7eab`. Inspection shows the detailed rose neuron, spine-rich dendrites, soma, segmented axon and terminal arbor in the right secondary field while the brain stays central.
- Error and correction: Xcode's normal signing pass rejected File Provider metadata attached to the repo-local Derived Data bundle. That failed bundle was not accepted. The exact successful product was first copied to `/tmp` without extended metadata, signed, installed, launched, and captured; the full signing build was then repeated with Derived Data directly under `/tmp`, where it succeeded and passed strict verification. One manual proof-verifier call also passed the leading-dash route incorrectly; rerunning it with `--route=...` passed.
- Accuracy boundary: this is a magnified, author-created generic morphology. It is not microscopy, histology, patient tissue, a recording, membrane voltage, ion exchange, synaptic timing, disease, treatment, or quantitative electrophysiology. Neuroanatomy and clinical specialist review remain pending.
- XCAT gate: `Scripts/deploy_xcat.zsh` stopped before build, install or launch because paired device `613CC48C-A6AD-5170-A238-D518B6012491` was unavailable. Receipt: `Proof/xcat/20260828-210419/BLOCKED.md`. No physical-device result is claimed.
- Verdict: `IMPROVED :)` — the neuron lesson now presents a real named 3D biological object instead of a handful of generated tubes, while preserving the calm point-owned teaching composition.
- Blocker: Simulator evidence cannot establish headset depth, gaze-and-pinch targetability, wearer comfort or comprehension, anatomical quality, specialist acceptance, or clinical validity.
- Next safe action: have a neuroanatomist review the generic morphology and run one physical Vision Pro depth and targetability check before any patient-facing or medical-training claim.
- Layman equivalent: the neuron beside the brain is now an actual packaged 3D model with a cell body, receiving branches, tiny spines, insulated signal fiber and endings. The Mac proves it loads and renders; a specialist and headset wearer still need to judge whether it is anatomically and spatially good enough.

## 2026-08-28 22:03 SGT — a named neurovascular cellular reference replaces another symbol

- Target: extend point-led teaching beyond one neuron with one detailed, stroke-relevant cellular USDZ while keeping the normal whole-brain story calm. Budget: one biological-reference slice on `latest-arnav-build`; no `main` merge, bulk asset load, paid provider use, patient data, operative simulation, device claim or clinical claim.
- One coherent action: authored and packaged an original magnified neurovascular-unit cutaway with nine named semantic parts, then connected **Brain-blood interface** to that model as a self-contained spatial reference. The normal path loads the USDZ; a visibly simpler tube-and-cell construction is reserved for load failure. The selected reference suppresses a duplicate drawer and keeps the registered whole brain, family rail and three-act timeline present.
- Evidence: `usdchecker --arkit` passes for source USDC and packaged USDZ. The 656,437-byte asset contains 7,272 vertices, 8,030 polygons and nine materials; SHA-256 `be82d271a33e3dee5c1c451384eccc84a783092b21ecb6ba3f3c8b1c4c1ed271`. `python3 Tests/verify_contract.py` passes with 36 explicit runtime USDZ files and the audited 134-record catalog unchanged. Both Simulator builds succeed, strict code-sign verification passes, and `Proof/122-family-neurovascular-unit-usdz-simulator.png` passes quality and route OCR at 3840×2160; image SHA-256 `111ae763793b9518f9b7d69545c16dd83feeb656cb939158eee94bdd1e3722fd`.
- Visual inspection: the left secondary field contains the authored capillary cutaway with luminal red cells, endothelial/junction cues, astrocyte and pericyte relationships and a nearby neuron. It is distinct from the dominant central whole brain and paired with one point-owned explanation rather than a permanent label cloud.
- Error and correction: the first screenshot was visually correct but OCR expected wording from the secondary reference title rather than the visible point-owned card. That receipt failed. The route contract was changed to two phrases actually visible in the inspected frame and then passed without relaxing image thresholds.
- Accuracy boundary: this is magnified, author-created generic morphology, not microscopy, histology, patient tissue, a patient scan or a measured blood-brain-barrier model. It does not simulate permeability, oxygen delivery, pressure, velocity, perfusion, disease or treatment. Neuroanatomy and clinical specialist review remain pending.
- XCAT gate: `Scripts/deploy_xcat.zsh` stopped before build, install or launch because paired device `613CC48C-A6AD-5170-A238-D518B6012491` was unavailable. Receipt: `Proof/xcat/20260828-213915/BLOCKED.md`. No wearer or device result is claimed.
- Verdict: `IMPROVED :)` — the family lesson now has a second real, component-named biological USDZ instead of another tube-and-sphere placeholder, without bulk-loading the larger catalog or crowding the brain story.
- Blocker: Simulator evidence cannot establish anatomical morphology quality, physical depth, gaze-and-pinch targetability, readability, wearer comprehension, specialist acceptance or clinical validity.
- Next safe action: have a neuroanatomist review the neuron and neurovascular morphologies together and perform one physical Vision Pro depth and targetability check before any patient-facing or medical-training claim.
- Layman equivalent: touching the new brain-blood-interface lesson now opens a packaged 3D capillary neighbourhood with blood cells, vessel-wall cells, support cells and a nearby neuron. The Mac proves it loads and looks distinct; a specialist and headset wearer still need to decide whether it is accurate and comfortable enough.
