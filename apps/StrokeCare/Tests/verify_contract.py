...
require("Enter the case room" in launch and "Turn left for patient files" in launch and "enterSpatialCaseRoom" in launch, "role-separated spatial threshold is missing")
...
require("StrokePointField" in state and "Regions" in state and "Procedure" in state, "clinician point-field flavors are missing")
...
require("regionPointDirections" in scene and "procedurePointPositions" in scene, "ten-point spatial reference data is missing")
require("experience.audienceLens == .clinician" in scene and "experience.pointField" in scene, "point fields are not clinician-only or switchable")
require("visualBounds(relativeTo: registered)" in scene and "* 0.98" in scene and "radius: 0.00165" in scene, "point fields are not derived from registered anatomy bounds at a precise scale")
...
require("generateSphere(radius: 0.0045)" in scene and "HoverEffectComponent" in scene, "point interaction affordance is missing")
...
require("--proof-family-question" in xcat_deploy and "WEARER_RESULT.md" in xcat_deploy and "NOT RUN" in xcat_deploy, "XCAT launch is not tied to a deterministic wearer receipt")
