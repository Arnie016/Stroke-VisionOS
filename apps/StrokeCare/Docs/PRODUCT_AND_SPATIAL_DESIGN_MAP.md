...
| --- | --- | --- | --- |
| Current Stroke Care USDZ build declarations | **13 unique USDZ paths** in both `project.yml` and the generated Xcode Resources phase: eight registered-v2 anatomy/context assets, three quarantined prototype-v1 assets, and two presenter-only tools. | Static bundle intent only. It does not prove the files copied, loaded, rendered, fit, or were clinically appropriate. | Verify the clean-build product contains exactly the thirteen filenames, then profile the named routes. |
| Current runtime load paths | Source attempts eleven anatomy/context names in `makeImportedAnatomy()` and two generic tool names in `makeClinicianHeldTools()`. Deep structures/ventricles are gated to clinician Study-apart; authored flow is gated to a selected clinician Blood-flow point. **Successful-load count: pending instrumented audit.** | Static reachability; `try?` means a missing or invalid file may silently fall back. | Add a non-PHI runtime receipt listing load success/fallback by asset and route. |
